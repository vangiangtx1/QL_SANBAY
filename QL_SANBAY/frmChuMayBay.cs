...
namespace QL_SANBAY
{
    public partial class frmChuMayBay : DevExpress.XtraEditors.XtraForm
    {
        ...
        private bool CheckMADN()
        {
            ...
            if (resultMa == 1)
            {
                MessageBox.Show("Mã DN đã tồn tại !", "", MessageBoxButtons.OK);
                return false;
            }
            return true;
        }
        ...
        private void btnXoa_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (MessageBox.Show("Bạn có thực sự muốn xóa người này ?",
                "Xác nhận", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                this.sP_CHUMAYBAY_NGUOITableAdapter.Fill(this.dSSB.SP_CHUMAYBAY_NGUOI);
                MessageBox.Show("Xóa thành công chủ máy bay ! ", "", MessageBoxButtons.OK);
            }
        }
        ...
        private void btnXoaDN_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (MessageBox.Show("Bạn có thực sự muốn xóa doanh nghiệp này ?",
                "Xác nhận", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                try
                {
                ...
                this.sP_CHUMAYBAY_DOANHNGHIEPTableAdapter.Fill(this.dSSB.SP_CHUMAYBAY_DOANHNGHIEP);
                MessageBox.Show("Xóa thành công doanh nghiệp ! ", "", MessageBoxButtons.OK);
            }
        }
        ...
        private void btnGhi_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (panelControl5.Enabled)
            {
                if (txtCMND.Text.Trim() == "" || txtDiaChi.Text.Trim() == "" || txtHoTen.Text.Trim() == ""
                    || txtSDT.Text.Trim() == "")
                {
                    MessageBox.Show("Vui lòng nhập đầy đủ dữ liệu! ", "", MessageBoxButtons.OK);
                    return;
                }
                try
                {
