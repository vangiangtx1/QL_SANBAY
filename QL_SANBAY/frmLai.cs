...
namespace QL_SANBAY
{
    public partial class frmLai : DevExpress.XtraEditors.XtraForm
    {
        ...
        private bool CheckCMND_MALOAI()
        {
            ...
            if (resultMa == 1)
            {
                MessageBox.Show("Phi công đang được chỉ định lái loại này !", "", MessageBoxButtons.OK);
                return false;
            }
            return true;
        }
        ...
        private void btnChiDinh_Click(object sender, EventArgs e)
        {
            ...
            if (CheckCMND_MALOAI())
            {
                if (MessageBox.Show("Bạn có thực sự muốn chỉ định phi công lái ?",
        ...
        {
            if (MessageBox.Show("Bạn có thực sự muốn hủy phi công lái loại máy bay này này?",
                    "Xác nhận", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                try
                {
                ...
                this.sP_DANHSACH_LAITableAdapter.Fill(this.dSSB.SP_DANHSACH_LAI);
                MessageBox.Show("Hủy sở hữu thành công! ", "", MessageBoxButtons.OK);
            }
        }
    }
}